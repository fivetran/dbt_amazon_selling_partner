...
{% enddocs %}
{% docs methods %} "Comma-separated list of payment methods used for an order. Each value may be COD (Cash on delivery) or GC (Gift card) or CVS (Convenience Store) or PointsAccount (Amazon Points) or Invoice or Other."{% enddocs %}
...
{% docs count_promotions_used %} The count of promotions used. {% enddocs %}

{% docs parent_variation_asin %} Identifier of the items variation parent. {% enddocs %}
